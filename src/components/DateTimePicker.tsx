...
export default function DateTimePicker({
  ...
  function commit(d: Date) {
    const clamped = !allowFuture && d > now ? now : d
  ...
  const canGoForward = allowFuture || new Date(viewYear, viewMonth + 1, 1) <= now
  const triggerHeight = size === 'sm' ? 'h-9 text-[13px]' : 'h-10 text-sm'
    ...
    <div className={className} ref={containerRef}>
      ...
      <div className="relative">
        <button
          type="button"
          onClick={() => setOpen(o => !o)}
          className={`w-full ${triggerHeight} px-3 pr-9 rounded-lg border bg-neutral-0 text-left flex items-center focus:outline-none focus:ring-2 focus:ring-brand-500 ${
          ...
          <div className="absolute z-50 mt-1 w-[280px] bg-neutral-0 border border-neutral-200 rounded-xl shadow-lg p-3 animate-popover">
            ...
            <div className="grid grid-cols-7 gap-y-0.5">
              {cells.map((day, i) => {
                ...
                const isDisabled = !allowFuture && day > todayStart
